...
在选项中加入`-floop-sve-mode-opt`即可。

注：仅 aarch64 且使能 SVE 时生效。
...
CFGO 在使用 PGO profile 的基础上联动使能一组 openEuler 优化；CSPGO 在内联后二次插桩，使 profile 计数携带调用上下文。

#### 使用方法

- CFGO 插桩：`-fcfgo-profile-generate[=<dir>]`；使用：`-fcfgo-profile-use[=<dir>]`。
- CSPGO 插桩：`-fcfgo-csprofile-generate[=<dir>]`；使用：`-fcfgo-csprofile-use[=<dir>]`；目录：`-fcfgo-csprofile-dir=<dir>`。
